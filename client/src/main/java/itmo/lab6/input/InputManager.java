...
public final class InputManager implements AutoCloseable {
    ...
    public void pushScript(String rawPath) throws InputException {
        ...
        Path scriptPath;
        try {
            scriptPath = Paths.get(rawPath.trim()).toRealPath();
        } catch (IOException e) {
            throw new InputException("Cannot open script: " + rawPath, e);
        ...
        if (scriptStack.size() >= 32) throw new InputException("Script nesting limit is 32");
    ...
    /** Stops queued scripts when the outcome of a network command is unknown. */
    public void cancelScripts() {
        while (!scriptStack.isEmpty()) closeCurrentScript();
    }
